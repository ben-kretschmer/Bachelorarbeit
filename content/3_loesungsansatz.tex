...
Ein Kriterium kann entweder erfüllt oder nicht erfüllt sein. Zunächst ist die einzige Komponente für ein erfülltes Kriterium die Existenz eines Produktes von Seiten des Anbieters. Dadurch ist die Variable $x$ (kurz für: Existenz beziehungsweise englisch existence) mit zwei Zuständen versehen:
\begin{eqnarray}
x_{k} \in \{0,1\}
\end{eqnarray}
...
Individuell zu jedem Kriterium wird ein Gewicht zugeordnet. Das Gewicht (englisch: weight) kann prinzipiell eine beliebige rationale Zahl sein:
\begin{eqnarray}
w_{k} \in \mathbb{Q}
\end{eqnarray}
...
Diese Faktoren werden integriert. Die Variable $t$ (kurz für: Technologie beziehungsweise englisch technology) repräsentiert die Zustände interne, Standard- oder offene Technologie beziehungsweise Software.
\begin{eqnarray}
t_{k} \in \{0,1,2\}
\end{eqnarray}

Der Umfang der Dokumentation wird mit der Variable $d$ und den zwei Zuständen vorhanden oder nicht vorhanden modelliert.
\begin{eqnarray}
d_{k} \in \{0,1\}
\end{eqnarray}
...
\begin{eqnarray}
s_{k} = w_{k}*x_{k}*(d_{k} + t_{k})
\end{eqnarray}

\paragraph{Effekt auf die Höchstpunktzahl}
Um die Höchstpunktzahl zu berechnen, müsste für $s'_{k}$ (von \ref{s-strich-von-k}) das Maximum gewählt werden, was drei entspricht. 
Die maximale Punktzahl würde also der Produktkatalog eines Cloud-Anbieters bekommen,
...
so ist die maximal mögliche Punktzahl bei einheitlich einfacher Gewichtung 300:

\begin{eqnarray}
S_{Maximal100} = \displaystyle\sum_{n=1} ^{100} w_{k}*x_{k}*(d_{k} + t_{k}) = \displaystyle\sum_{n=1} ^{100} 1*1*(1+2) = 100 * 3 = 300
\end{eqnarray}

\paragraph{Charakterisirung eines Anbieters mit keinen Punkten}
...
\begin{eqnarray}
S_{Minimal100} = \displaystyle\sum_{n=1} ^{100} w_{k}*x_{k}*(d_{k} + t_{k}) = \displaystyle\sum_{n=1} ^{100} 1*1*(0) = 100 * 0 = 0
\label{Minimal100}
\end{eqnarray}
...
In Relation zu den im Kapitel \ref{Gartner-Magic-Cloud} aufgeführten Cloud-Anbietern, entspricht die Stichprobengröße von 4 rund 20\% der Gesamtzahl der größten Anbieter.
...
Produkte, die eine Bedienoberfläche zur Überwachung oder der Verwaltung von Ressourcenverbauch 
